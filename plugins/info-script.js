...
const handler = async (m, { conn, args }) => {
   const res = await fetch('https://api.github.com/repos/kenn/bennbyx-MD');
   const json = await res.json();
   let txt = `           *🍁  S C R I P T  🍁*\n\n`;
      txt += `◦  *Nombre:* ${json?.name || '𝗢𝗡𝗬𝗫'}\n`;
      ...
      txt += `◦  *Url:* ${json?.html_url || 'https://github.com/kennx/onyxbyx'}\n\n`;
      txt += `	 ${json?.forks_count || '-'} Forks · ${json?.stargazers_count || '-'} Stars · ${json?.open_issues_count || '-'} Issues`;
   await conn.sendMessage(m.chat, {text: txt.trim(), mentions: [...txt.matchAll(/@([0-9]{5,16}|0)/g)].map((v) => v[1] + '@s.whatsapp.net'), contextInfo: {forwardingScore: 9999999, isForwarded: true, mentionedJid: [...txt.matchAll(/@([0-9]{5,16}|0)/g)].map((v) => v[1] + '@s.whatsapp.net'), "externalAdReply": {"showAdAttribution": true, "containsAutoReply": true, "renderLargerThumbnail": true, "title": global.titulowm2, "containsAutoReply": true, "mediaType": 1, "thumbnail": imagen6, "mediaUrl": `https://www.atom.bio/onyx-bot`, "sourceUrl": `https://www.atom.bio/onyx-bot`}}}, {quoted: m});
};
handler.help = ['scbot'];
handler.tags = ['info'];
